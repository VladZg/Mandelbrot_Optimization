...
- Evaluating the impact of the compilation flags on the programms perfomances, the table shows that on powerful flags ``-O2``, ``-O3``, ``-Ofast`` the acceleration is the most significant and it is about **40**, **19**, **27** times compared to programs without flags, but using same SIMD-instructions. At the same time, their impacts don't differ so much from each other. The work of programms with the ``-O1`` flag is already very different, when using it speed increases about **3-4** times comparing to programms with the same SIMD-instructions but without optimization flags.
